...
before pac is from output???
```{r}
unique(pac$post_acute_trherapy)
```

what different categories and hw many?


```{r}
lenght(unique(pac$post_acute_trherapy))
```
to keep only unique rows
```{r}
...
Select first three columns: alwaya works on columns
```{r}
select(viruses, 1:3)
```
...
```{r}
select(summary_csv,starts_with("acute"))
```

!!! Only uses exact expressions
...
```{r}
select(summary_csv,ends_with("LOS"))
```
...
```{r}
select(summary_csv,contains("date"))
```
...
at least one number
```{r}
select(summary_csv, matches("[0-9]"))
```
2 numbers
```{r}
select(summary_csv, matches("[0-9]{2}"))
```

1 or 2 numbers
```{r}
select(summary_csv, matches("[0-9]{1,2}"))
```
...
everything select every column in the given order
...
```{r}
?select
select(viruses, contains("GC", ignore.case = FALSEremaind))
```