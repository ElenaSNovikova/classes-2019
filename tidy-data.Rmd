...
```{r}
(index <- read_csv(file = "data/viruses.csv", col_types=cols(release_date="c")))
```
...
can modify 1 column
```{r}
read_csv("a,b\n4,2-13-2017",col_types=cols(b=col_date(format="%m-%d-%Y")))
```
...
read_csv("# A comment I want to skip
  ...
  1,2,3", comment = "#")
```
comment will be skiped automatically
...
(pac <- read_delim("hf-data/post-acute_care_data_only (rows).csv",
                  na="n/a",
                  col_types=cols(invoice_start=my_date_format,
                                 invoice_end=my_date_format)))
```
numbers will be conducted to characters. if n/a
...
```{r}
(z_500 <- read_excel("hf-data/post-acute_care_data_only (rows).xlsx", sheet = "post-acute_care"))
```
Problem with dates
...
```
can asign the name to a sheet. can also do for 1 sheet
