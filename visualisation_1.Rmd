...
Is everything OK with shapes? As ther are only 6 shapes initially
...
```
Use log function natural log or log10

```{r}
ggplot(data = starwars) +
  geom_point(mapping = aes(x = height, y = log(mass), size = birth_year))
```
