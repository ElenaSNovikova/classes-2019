...
Using mpg dataset, we **plot each class of cars on a separate subplot** and we arrange plots into 2 rows: class to split the data set to change order need to change data set
```{r}
ggplot(data = mpg) +
...
Here, we create displ~hwy scatterplot with loess smooth line fitted trough all point, additionally we map color to the class of cars (whereas geom_smooth still plots only one line):
...
```{r}
ggplot(data = diamonds_summarised)+
  stat_count(mapping=aes(cut, y=..prop.., group = 1))
```
...
Mean and SE: more that 1 data pont
```{r}
ggplot(data = diamonds) + 
  stat_summary(
    mapping = aes(x = cut, y = depth),
...
ggplot(data = diamonds) + 
  stat_summary(
    mapping = aes(x = cut, y = depth),
geom="arrorbar"+
stat_summary(mapping = aes(x = cut, y = depth),fun.y = mean, geom="point")
```
...
1. What does labs() do? Read the documentation. add different labels
